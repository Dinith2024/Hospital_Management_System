...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$nic) {
        $response['message'] = 'Please enter your NIC.';
        echo json_encode($response);
        exit;
    }

    // Lookup patient
    $stmt = $conn->prepare("SELECT `PatientID`, `Email Address`, `First Name` FROM patients WHERE `NIC` = ?");
    ...
    if ($result->num_rows === 1) {
		...
		$stmt2 = $conn->prepare("
			...
			VALUES (?, 'patient', ?, NOW() + INTERVAL 1 HOUR)
		");
		...
		$resetLink = "http://localhost/$folder/PHP/ResetPassword.php?token=$token&uid=$patientID";
        ...
        $response['message'] = 'If a patient with that NIC exists, a password reset email has been sent.';
    } else {
